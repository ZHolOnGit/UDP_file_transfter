...
1.1. Run the test programs

The simplest way to build and run the test programs is to enter the
following command:

    ./runtests.sh
    
this will compile test programs and run them. It uses the same test
framework as was used for assignment 1 and output is similar.

Enter:

    ./runtests.sh -h
    
for a usage message and how to test separate parts of the assignment.

Some tests will block if there are errors because they run two 
processes for the client and server and, if the client does not
successfully send data the server will block waiting.
...
run the applications:
    ...
    run-client.sh to run the rft-client
